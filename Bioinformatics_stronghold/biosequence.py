...
class Biosequence():
    ...
    def reverse_complement_sequence(self):
        ...
        return ''.join(self.complement_sequence())[::-1]
    ...
    def translate_sequence(self):
        ...
        codon_len = 3
        protein = []
        for i in range(0, len(self.seq)-codon_len+1, codon_len):
    ...
    def adjacency_list(self, seqs_dict=None, k=3):
        """Returns a list of adjacent edges of a dictionary of sequences"""
        if seqs_dict is not None:
            target_seq_dict = seqs_dict
        else:
            target_seq_dict = self.seq
    ...
    def get_mRNA_seqs_from_protein(self, protein_seq):
        """Returns the total number of mRNA sequences that can possibly translated a protein sequence (modulo 10**6)"""
        if protein_seq is not None:
            full_protein_seq = protein_seq + '_'
        else:
